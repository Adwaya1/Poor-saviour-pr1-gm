...
function preload(){
  //background images for the levels
    ...
    backImg4 = loadImage("Images/crowdcheering.jpg");
    ...
    homelessManImage = loadImage("Images/location.png")
...
cityGroup3.setVisibleEach(false);
 ...
 homelessMan.addImage(homelessManImage);
 homelessMan.scale = 0.3;
...
function draw() {
  ...
  if(level === 1){
    ...
    if(trainGroup.isTouching(foodtruck)){
      foodtruck.x = width-10;
      foodtruck.y = 500;
    }
  }
   ...
   // foodtruck.y = 230;
    ...
    background(backImg4); 
    homelessMan.visible =false;
    foodtruck.visible = false;

    textSize(35);
fill("red");
stroke("yellow");
strokeWeight(6);
text("You are a CHAMPION",width/2 -200,height/3);
  }
...
stroke("yellow");
strokeWeight(6);
if(level !== 3)text("Level Completed, Press Space to Level Up",width/2 -200,height/2);
else text("You Succesfully completed all the levels",width/2 -200,height/2);
if(keyDown("space")){
  level = level+1;
  gameState = 0;
}
